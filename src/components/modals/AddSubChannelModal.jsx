...
const AddSubChannelModal = ({setShowModal}) => {
    const [input,setInput] = useState({subChannelName:"",isPrivate:false});
    const [flag,setFlag] = useState(false);

    const isChannelDisabled = () => input.subChannelName.length>25 || input.subChannelName===""

    const clickHandler=()=>
    {
        if(flag)
        {
            // Enter the actual function to call create sub channel API
            console.log(input);
            setShowModal(false);
        }
        else {
  ...
  return (
    <div className='fixed bg-black/30 w-screen h-screen z-40 top-0 flex justify-center items-center'>
        <main className='relative flex flex-col items-start bg-white  min-h-fit max-h-max min-w-[25%] rounded-lg shadow-lg pt-3 pl-5 pr-7 pb-5 max-sm:h-screen max-sm:justify-center'>
            ...
            <h1 className='text-3xl font-bold'>Create a Sub-Channel</h1>
            {flag && <small htmlFor='name' className='text-text'>#{input.subChannelName}</small>}
            ...
            <input value={input.subChannelName} className='w-full py-3 px-2 border rounded-md' placeholder='# Enter sub-channel name' onChange={(e)=>setInput(prev=>({...prev,subChannelName:e.target.value}))} />
            <span className={`absolute right-2 top-3 ${input.subChannelName.length >= 25?'text-red-500':'text-text'}`}>{(25 - input.subChannelName.length)}</span>
            </aside>}
            {!flag && <small className='max-w-md text-[#525252] mt-1.5'>Sub-channels are focused topics within a channel for streamlined discussions. Clearly named for easy navigation. </small>}
...
export default AddSubChannelModal